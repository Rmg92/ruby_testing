...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        user_input = 5
        verified_input = verify_input.verify_input(min, max, user_input)
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_create) { described_class.new(1, 10) }
    let(:binary_create) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(binary_create).to receive(:make_guess)
      allow(binary_create).to receive(:update_range)
      allow(game_create).to receive(:display_guess)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_create.display_turn_order(binary_create) }.to change { game_create.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_create).to receive(:make_guess).once
      game_create.display_turn_order(binary_create)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(game_create).to receive(:display_guess).once
      game_create.display_turn_order(binary_create)
    end
